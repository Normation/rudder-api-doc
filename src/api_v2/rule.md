 ...
 */
 ...
 /**
   ...
   @apiParam (Multi) {String} [targets] Link one target to the Rule. Rule targets are "group:groupId" "node:nodeId" and the special targets : "special:all" and "special:all_exceptPolicyServers"
 */
...
-----------------
    ...
    curl -H "X-API-Token:cEMKWn38wzgpIZPwmipe1NPeAmfzwroR" -X GET http://serverRudder/rudder/api/2/rules?prettify=true
...
-----------------
    ...
    @apiExample Example usage:
    curl -H "X-API-Token:TokenId” -X PUT http://serverRudder/rudder/api/2/rules -d "displayName=Name of New Rule"
...
-----------------
    ...
    @apiExample Example usage:
    curl -H "X-API-Token:TokenId" -X PUT http://serverRudder/rudder/api/2/rules?prettify=true  -d "source=RuleIDSource" -d "displayName=Name of New Rule”
...
--------------------------
    ...
    @apiExample Example usage:
    curl -H "X-API-Token:TokenId" -X GET http://serverRudder/rudder/api/2/rules/06ba8940-ed6c-4102-ba46-93d640a64c36?prettify=true
    @apiSuccessExample Success-Response:
      HTTP/1.1 200 OK
      {
        "action":"ruleDetails",
...
--------------------------
    ...
    @apiExample Example usage:
    curl -H "X-API-Token:TokenId" -X DELETE http://serverRudder/rudder/api/2/rules/176ad06b-ed02-4da3-8053-16225d217741?prettify=true
    @apiSuccessExample Success-Response:
      HTTP/1.1 200 OK
      {
        "action":"deleteRule",
...
--------------------------
    ...
    @apiExample Example usage:
    Update display name: 
    curl -H "X-API-Token:TokenId" -X POST http://serverRudder/rudder/api/2/rules/RuleID -d "displayName=Name of Rule"
